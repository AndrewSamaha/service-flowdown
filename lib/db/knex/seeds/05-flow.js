...
exports.seed = async function(knex) {
  ...
  const song_ids = await knex('songs').insert([
    ...
    {
      name: 'Follow the Sun',
      artist: 'Caroline Pennell, JordanXL',
      duration: (3+30/60),
      owner_id
    },
    {
  ...
  const flow_ids = await knex('flows').insert([
    {
      name: '6:15 Hot Power Tuesday 2024-01-09',
      owner_id
    }]
  ).returning('id');
